...
- `PUT /api/v1/device/protections` тело `{"ovp"?: 31.0, "ocp"?: 5.2, "opp"?: 155.0,
  ...
  otp ≤ 80, lvp ≥ 0 → иначе `400 invalid_protection`.
...
Сообщение `event` (v1) переносит те же kinds, что и журнал.
`telemetry.data.metering` уже в v1. Новых типов сообщений нет.
